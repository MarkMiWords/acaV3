...
export function checkText(text: string): GuardrailResult {
  ...
  if (detectInstructionalHarm(normalised)) {
    return block(
      "instructional_harm",
      "Instructional or enabling harm is not permitted."
    );
  }
  ...
  if (detectTargetedThreat(normalised)) {
    return block(
      "targeted_threat",
      "Targeted threats or incitement are not permitted."
    );
  }
 ...
 */
export function safeResponseFor(category: GuardrailCategory): string {
  switch (category) {
    case "self_harm":
      return (
    ...
    case "instructional_harm":
      return (
        "I can help you write about experiences, feelings, or consequences, " +
        "but I can’t assist with instructions or guidance that could cause harm."
      );

    case "targeted_threat":
      return (
        "I can help with reflective or narrative writing, " +
        "but I can’t assist with threats or harm toward specific people or groups."
      );

    case "sexual_minors":
      return (
        "I can’t engage with or assist in content involving sexual harm to minors."
      );

    default:
      return (
        "I can help with your writing, but I can’t continue with that request as written."
      );
  }
}
 ...
 */
function detectInstructionalHarm(text: string): boolean {
  const patterns = [
 ...
 */
function detectTargetedThreat(text: string): boolean {
  const patterns = [
    "i will kill you",
    "i am going to kill",
    "they deserve to die",
 ...
 */
function detectSexualMinors(text: string): boolean {
  const patterns = [
    "child pornography",
    "sexual with a minor",
    "underage sex",
...
function block(category: GuardrailCategory, reason: string): GuardrailResult {
  return {
    allowed: false,
    category,
    reason
